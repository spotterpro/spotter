// 📁 lib/src/screens/my_growth_log_screen.dart
...
class MyGrowthLogScreen extends StatelessWidget {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildXpSystemInfo(context),
            const SizedBox(height: 32),
            _buildSectionTitle('XP 획득 방법'),
            const SizedBox(height: 16),
            _buildXpMethodsWrap(context),
            const SizedBox(height: 32),
          ...
          children: [
            ...
            Center(
              child: Column(
                ...
                children: [
                  const Text('LV.', style: TextStyle(color: Colors.grey)),
                  Text('${userProfile.level}', style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold)),
                  Text('${userProfile.currentXpInLevel} / ${userProfile.xpForNextLevel} XP', style: const TextStyle(color: Colors.grey)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildXpSystemInfo(BuildContext context) {
    return Container(
  ...
  Widget _buildXpMethodsWrap(BuildContext context) {
    final List<Widget> xpCards = [
      ...
      const _XpMethodCard(icon: Icons.chat_bubble_outline, title: '커뮤니티 활동', xp: '+2 ~ +10 XP'),
    ];
  ...
  Widget _buildLevelTitlesList(BuildContext context) {
    return Column(
      children: [
        _LevelTitleTile(icon: Icons.child_friendly, level: 1, title: '새내기 스포터', isUnlocked: userProfile.level >= 1),
        _LevelTitleTile(icon: Icons.explore_outlined, level: 10, title: '동네 탐험가', isUnlocked: userProfile.level >= 10),
        _LevelTitleTile(icon: Icons.emoji_emotions_outlined, level: 25, title: '골목대장', isUnlocked: userProfile.level >= 25),
        _LevelTitleTile(icon: Icons.business_outlined, level: 50, title: '도시 개척자', isUnlocked: userProfile.level >= 50),
        _LevelTitleTile(icon: Icons.workspace_premium_outlined, level: 100, title: '스팟 마스터', isUnlocked: userProfile.level >= 100),
      ],
    );
  }
}
...
class _LevelTitleTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      color: isUnlocked ? Colors.orange.withOpacity(0.1) : Theme.of(context).cardColor.withOpacity(0.4),
