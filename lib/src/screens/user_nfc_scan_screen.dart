...
import 'package:spotter/src/screens/upload_feed_screen.dart';
...
class _UserNfcScanScreenState extends State<UserNfcScanScreen> {
  ...
  Future<void> _processReward() async {
    ...
    await _db.collection('users').doc(_currentUserId).collection('stamps').add({
      ...
      'rewardTitle': widget.rewardData['title'],
      'storeName': widget.rewardData['storeName'],
    ...
    if (newProgress >= requiredStamps) {
      ...
      if (mounted) {
        ...
        final wantsToWritePost = await _showPostCreationDialog('쿠폰 획득!');
        if (wantsToWritePost ?? false) {
          final currentUserSnap = await _db.collection('users').doc(_currentUserId).get();
          final currentUserMap = currentUserSnap.data() ?? {};
          // --- 🔥🔥🔥 수정된 부분: certifiedStoreId 파라미터를 제거했습니다. ---
          Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => UploadFeedScreen(
            currentUser: currentUserMap,
          )));
        } else {
          mainScreenNavigator.value = 2;
      ...
      if (mounted) {
        await Future.delayed(const Duration(seconds: 1));
        final wantsToWritePost = await _showPostCreationDialog('스탬프 적립 완료!');
        if (wantsToWritePost ?? false) {
          final currentUserSnap = await _db.collection('users').doc(_currentUserId).get();
          final currentUserMap = currentUserSnap.data() ?? {};
          // --- 🔥🔥🔥 수정된 부분: certifiedStoreId 파라미터를 제거했습니다. ---
  ...
  Future<bool?> _showPostCreationDialog(String title) {
    return showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(title),
        content: const Text('이 방문을 피드로 남겨 인증 뱃지를 획득하시겠습니까?'),
        actions: [
          TextButton(
