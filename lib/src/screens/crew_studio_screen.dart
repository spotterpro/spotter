...
class _CrewStudioScreenState extends State<CrewStudioScreen> with TickerProviderStateMixin {
      ...
      child: Column(
        ...
        children: [
          ...
          GridView.count(
            ...
            children: const [
              _InfoCard(icon: Icons.arrow_upward, title: '나의 레벨', value: 'LV.25', color: Colors.green),
              _InfoCard(icon: Icons.flash_on, title: '경험치(XP)', value: '1,530', color: Colors.orange),
              _InfoCard(icon: Icons.business_center_outlined, title: '진행중인 스폰서쉽', value: '1', color: Colors.blue),
              _InfoCard(icon: Icons.check_circle_outline, title: '총 태깅 발생', value: '1,284', color: Colors.purple),
            ],
          ),
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => StoreDetailScreen(
                    storeData: store['storeData'] as Map<String, dynamic>,
                  ),
                ),
              );
            },
              ...
              child: Row(
                children: [
                  ClipRRect(
                    ...
                    child: Image.network(
                      'https://picsum.photos/seed/${store['seed']}/100/100',
                      width: 60,
                      height: 60,
                      fit: BoxFit.cover,
                    ),
  ...
  Widget _buildRankingSection(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Column(
        children: [
          TabBar(
            isScrollable: false,                       // 균등폭 고정
            indicatorSize: TabBarIndicatorSize.tab,    // 인디케이터도 탭 폭 기준
            labelColor: on,
            unselectedLabelColor: Colors.grey,
  ...
  Widget _buildRankingList(BuildContext context, String type) {
    final rankings = type == '스탬프왕'
        ? [
      {'name': '스포터', 'metric': '스탬프 123개', 'seed': 'user1'},
      {'name': '먹깨비', 'metric': '스탬프 115개', 'seed': 'user2'},
      {'name': '헬창', 'metric': '스탬프 98개', 'seed': 'user3'},
    ]
        : [
      {'name': '동성로 탐험대', 'metric': '크루원 25명', 'seed': 'crew1'},
      {'name': '앞산 등산모임', 'metric': '크루원 18명', 'seed': 'crew2'},
      {'name': '수성못 야경단', 'metric': '크루원 12명', 'seed': 'crew3'},
    ];
  ...
  Widget _buildStoreRankingList(BuildContext context) {
    final rankings = [
      {'name': '맛집 파스타', 'metric': '단골 127명', 'seed': 'pasta', 'storeData': {'storeName': '맛집 파스타', 'regulars': 127, 'seed': 'pasta'}},
      {'name': '카페 스프링', 'metric': '단골 98명', 'seed': 'cafe', 'storeData': {'storeName': '카페 스프링', 'regulars': 98, 'seed': 'cafe'}},
      {'name': '클린 세탁소', 'metric': '단골 75명', 'seed': 'laundry', 'storeData': {'storeName': '클린 세탁소', 'regulars': 75, 'seed': 'laundry'}},
    ];
    return ListView.builder(
      padding: const EdgeInsets.only(top: 8),
      itemCount: rankings.length,
      ...
      itemBuilder: (context, index) {
        return ListTile(
          onTap: () => Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => StoreDetailScreen(
                storeData: rankings[index]['storeData'] as Map<String, dynamic>,
              ),
            ),
          ),
          leading: Text('${index + 1}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.orange)),
